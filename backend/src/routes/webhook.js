...
function extractWhatsAppMessage(body) {
  ...
  const elements = interactive?.nfm_reply?.elements || interactive?.nfm_reply?.response_json?.elements || interactive?.nfm_reply?.body?.params || [];

  const form_fields = Object.fromEntries((elements || []).filter((e) => e && e.name).map((e) => [e.name, e.value ?? e.text ?? e.answer ?? e.input ?? ""]));
...
async function getOpenTicketsForUser(userId) {
  if (!userId) return [];
  const r = await dbQuery("SELECT * FROM tickets WHERE raised_by_user_id = $1 AND status IN ('OPEN','ASSIGNED','IN_PROGRESS') ORDER BY created_at DESC LIMIT 5", [userId]);
  return r?.rows || [];
}

function formatTicketSummary(tickets) {
  if (!tickets?.length) return "No open complaints found.";
  const lines = tickets.map((t) => `${t.id || t.ticket_id || "#"}: ${t.status || "OPEN"} - ${t.category || ""}`);
  return `Your open complaints:\n${lines.join("\n")}`;
}
...
router.post("/whatsapp", async (req, res) => {
  try {
    ...
    // This keeps inbound users on the flow path instead of freeform text.
    ...
    if (flowEnabled && flowTemplateName && normalized.message_type !== "interactive" && normalized.message_type !== "status") {
    ...
    if (intent === "CREATE_TICKET") {
      ...
      if (normalized.sender_whatsapp_number) {
        const tickets = await getOpenTicketsForUser(identity.user_id);
        const reply = tickets.length ? formatTicketSummary(tickets) : process.env.WHATSAPP_TEXT_REPLY || "Please choose a service: 1) Electrical 2) Plumbing 3) Cleaning 4) AC Service";
        sendWhatsAppText(normalized.sender_whatsapp_number, reply).catch((e) => log("WhatsApp text send error", e?.message || e));
      }
    }
